...
@router.delete("/exchange/{exchange_name}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_info(exchange_name: str, delete_case: DeleteExchangeUseCase = Depends(delete_use_case)):
    await delete_case.delete_exchange_info(exchange_name=exchange_name)
